...
class ClusterCreation:
    ...
    def create_cluster(self, cluster_name):
        try:
            ...
            output = self.run_ansible_playbook(ansible_command)

            # Check the output to determine success or failure
            if "success" in output.lower():
                return {'status': 'success', 'message': 'Cluster creation request sent successfully', 'cluster_name': cluster_name}
            else:
                return {'status': 'error', 'message': 'Cluster creation failed', 'cluster_name': cluster_name}, 400
