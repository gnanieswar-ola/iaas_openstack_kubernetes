...
def create_dynamic_inventory_api():
    ...
    # Validate master_ips and worker_ips
    try:
        master_ips = [str(ipaddress.ip_address(ip)) for ip in master_ips]
        worker_ips = [str(ipaddress.ip_address(ip)) for ip in worker_ips]
